@extends('layouts.main')
@section('title', 'Welcome')
@section('content')

    <div class="tp-service-2__area tp-service-2__space fix p-relative pt-50" style="margin-top: 0px;">
        <div class="tp-service-2__shape-4 d-none d-xl-block">
            <img src="{{ asset('assets/img/feature/shape-2-2.png') }}" alt="">
        </div>

        <div class="tp-service-2__shape-5 d-none d-xl-block">
            <img src="{{ asset('assets/img/service/shape-3-4.jpg') }}" alt="">
        </div>

        <div class="container custom-container-1">
            <div class="row">
                <div class="col-xl-12">
                    <div class="tp-service-2__top-text d-none d-lg-block pb-75 text-center">
                        <h2 class="tp-service-2__big-title">
                            Shwe Wutt Hmone Kha
                        </h2>
                    </div>
                </div>
            </div>

            <div class="row">
                <div class="col-xl-6 col-lg-6">
                    <div class="tp-service-2__thumb-box p-relative text-end">
                        {{-- <div class="tp-service-2__main-thumb">
                        <img src="assets/img/service/service-3-1.jpg" alt="">
                    </div>
                    <div class="tp-service-2__thumb-sm">
                        <img src="assets/img/service/service-3-2.jpg" alt="">
                    </div>
                    <div class="tp-service-2__shape-1 d-none d-xl-block">
                        <img src="assets/img/service/shape-3-1.jpg" alt="">
                    </div>
                    <div class="tp-service-2__shape-2 d-none d-xxl-block">
                        <img src="assets/img/service/shape-3-2.jpg" alt="">
                    </div>
                    <div class="tp-service-2__shape-3 d-none d-xl-block">
                        <img src="assets/img/feature/shape-2-5.png" alt="">
                    </div> --}}

                        <img src="https://shwewutthmonekha.com/assets/data/about4.png" alt="" style="width: 100%;">
                    </div>
                </div>

                <div class="col-xl-6 col-lg-6 wow tpfadeRight" data-wow-duration=".9s" data-wow-delay=".7s">
                    <div class="tp-service-2__right-box">
                        <div class="tp-service-2__section-title mb-30">
                            <span class="tp-gradiant-section-subtitle">
                                About of Our Company
                            </span>
                            <h3 class="tp-section-title-3">
                                Shwe Wutt
                                <span class="p-relative">
                                    Hmone Kha
                                    <span class="tp-title-shape-4">
                                        <svg width="197" height="4" viewBox="0 0 197 4" fill="none"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
                                                d="M194.384 3.21727C194.988 3.21727 195.994 3.1893 196.598 3.24524C195.592 3.24524 194.586 3.27322 193.781 3.27322C193.378 3.32916 194.384 3.35713 194.787 3.3851C195.592 3.3851 196.396 3.3851 197 3.44105C196.396 3.46902 195.592 3.46902 194.787 3.46902C193.982 3.46902 192.976 3.44105 192.372 3.49699C192.976 3.52496 192.976 3.52496 192.976 3.58091C191.568 3.60888 191.568 3.77671 189.354 3.72077C188.55 3.72077 187.745 3.72077 186.94 3.72077C185.934 3.83266 184.727 3.94454 182.111 3.8886C181.307 3.91657 180.502 3.91657 179.697 3.8886C177.484 3.94454 175.271 3.91657 173.057 3.94454C172.856 3.97252 172.655 3.97252 172.454 3.94454C171.649 3.94454 170.844 3.94454 170.039 3.94454H169.838H169.637C168.832 3.94454 168.229 3.97252 167.424 3.94454C164.607 3.94454 161.589 3.94454 158.772 3.97252C156.962 3.97252 155.553 3.94454 154.547 3.86063H154.346H154.145C153.541 3.91657 153.742 4.00049 151.73 4.00049C149.718 3.97252 147.706 3.97252 145.493 3.97252C144.085 3.94454 142.676 4.00049 141.268 3.97252C136.238 3.94454 131.208 3.94454 126.178 3.91657C121.752 3.91657 117.325 3.8886 112.899 3.86063C111.29 3.91657 109.68 3.86063 108.272 3.86063C100.224 3.83266 91.9745 3.83266 83.9266 3.74874C80.305 3.72077 76.6834 3.74874 73.0619 3.6928C69.8427 3.6928 66.8247 3.66482 63.6055 3.63685C56.1612 3.58091 48.7168 3.52496 41.4737 3.41308C38.4557 3.41308 35.639 3.3851 33.0234 3.30119C28.7982 3.30119 24.7742 3.21727 20.9515 3.13336C20.3479 3.13336 19.9455 3.10538 19.5431 3.10538C17.9335 3.13336 16.7263 3.07741 15.3179 3.02147C14.312 3.02147 13.5072 2.9935 12.9035 2.96552C12.3 2.93755 11.6964 2.93755 11.0928 2.90958C10.4892 2.85364 9.48318 2.85364 8.47719 2.85364C6.8676 2.79769 4.85562 2.76972 4.45323 2.62986C1.43526 2.60189 0.630463 2.49 0.83165 2.35014C-0.375534 2.18231 -0.174332 2.01447 0.83165 1.84664C0.83165 1.76273 0.83165 1.67881 0.83165 1.59489V1.56692C0.42926 1.37112 2.64244 1.20329 2.03885 1.03545C1.83765 0.895593 3.04482 0.783705 3.84961 0.671817C3.84961 0.643845 3.84962 0.587901 4.65442 0.559929C4.85562 0.531957 5.45923 0.503985 6.26402 0.476013C7.27 0.364125 8.47717 0.280209 10.4892 0.252237C10.8916 0.196293 11.6964 0.224265 12.2999 0.224265C12.9035 0.196293 13.5071 0.16832 14.1107 0.140348C14.7143 0.112376 15.3179 0.112376 15.9215 0.112376C18.3359 0.0564323 20.7503 0.0284603 23.3659 0.0284603C24.3719 0.0284603 25.1767 0.0284603 26.1826 0.0284603C26.585 0.0284603 27.1886 0.0284603 27.591 0.000488281C33.0234 0.000488281 38.657 0.000488281 44.0893 0.0284603C46.9061 0.0284603 49.7228 0.000488281 52.5396 0.0844044C54.954 0.0844044 57.7708 0.0284603 60.3864 0.112376C60.9899 0.112376 61.5935 0.112376 61.9959 0.112376C63.8067 0.0844044 65.4163 0.0844044 67.2271 0.112376C71.6535 0.112376 76.0798 0.140348 80.305 0.16832C81.7134 0.16832 83.3229 0.140348 84.5301 0.196293C85.3349 0.224265 86.1397 0.224265 86.9445 0.196293C89.5601 0.196293 92.1757 0.196293 94.7913 0.224265C95.1937 0.224265 95.5961 0.224265 95.9985 0.224265C97.0044 0.252237 98.2116 0.224265 99.2176 0.252237C100.626 0.252237 102.236 0.252237 103.644 0.252237C104.851 0.252237 106.26 0.252237 107.266 0.280209C108.07 0.280209 108.875 0.280209 109.68 0.280209C111.29 0.280209 113.1 0.252237 114.71 0.308181C117.728 0.308181 120.746 0.336153 123.563 0.336153C128.391 0.336153 133.22 0.392097 138.049 0.420069C139.658 0.420069 141.067 0.420069 142.676 0.448041C141.268 0.448041 139.658 0.448041 138.25 0.448041C139.658 0.420069 141.268 0.420069 142.676 0.448041C145.292 0.448041 147.706 0.448041 150.121 0.503985C151.127 0.503985 152.133 0.503985 153.139 0.531957C153.34 0.531957 153.541 0.531957 153.742 0.531957C155.956 0.531957 158.169 0.559929 160.382 0.615873C162.394 0.615873 164.205 0.671817 166.015 0.699789C167.021 0.699789 168.027 0.699789 169.033 0.755733C170.844 0.783705 172.454 0.811677 173.057 0.895593C174.265 0.839649 175.069 0.895593 176.075 0.923565C177.081 0.979509 176.679 1.06343 177.886 1.11937C177.283 1.11937 176.679 1.11937 176.075 1.11937C176.277 1.14734 176.478 1.17531 176.679 1.17531C178.087 1.23126 179.496 1.25923 180.904 1.2872C181.105 1.2872 181.307 1.2872 181.508 1.2872C182.916 1.2872 184.526 1.25923 185.733 1.31517H185.934C185.934 1.31517 185.934 1.34315 185.733 1.34315C184.928 1.34315 184.123 1.34315 183.52 1.34315H183.319C184.325 1.31517 185.129 1.31517 185.733 1.34315C186.135 1.45503 184.928 1.48301 184.526 1.53895C184.727 1.56692 183.721 1.59489 184.526 1.62287C185.129 1.62287 186.135 1.59489 185.934 1.65084C185.733 1.70678 184.526 1.67881 183.922 1.67881C183.721 1.67881 183.52 1.70678 183.319 1.70678C184.928 1.7907 186.538 1.81867 188.147 1.87461C190.562 1.87461 191.568 1.9865 192.976 2.07042C193.378 2.09839 193.982 2.07042 194.183 2.09839C192.976 2.09839 193.58 2.18231 192.775 2.21028C189.958 2.26622 189.958 2.35014 192.574 2.46203C193.378 2.49 194.988 2.51797 195.39 2.60189C195.39 2.62986 195.39 2.65783 195.39 2.65783C195.793 2.65783 196.195 2.65783 196.195 2.6858C195.994 2.74175 195.39 2.71378 194.787 2.71378C194.384 2.71378 193.781 2.71378 193.58 2.74175C193.982 2.82566 193.781 2.85364 192.574 2.85364C192.976 2.90958 194.183 2.88161 193.982 2.96552C194.586 2.96552 194.988 2.9935 195.592 2.9935C194.988 3.04944 193.781 2.9935 193.58 3.04944C194.183 3.10538 194.183 3.13336 193.177 3.10538C192.372 3.10538 191.366 3.07741 192.372 3.16133C192.976 3.16133 193.781 3.16133 194.384 3.21727ZM5.66039 1.48301C5.45918 1.48301 5.4592 1.48301 5.4592 1.51098C5.4592 1.48301 5.66039 1.48301 5.66039 1.48301ZM56.3624 1.90259C56.5636 1.93056 56.5636 1.90259 56.7648 1.90259C56.5636 1.90259 56.3624 1.90259 56.3624 1.90259ZM81.5122 2.32217C81.311 2.32217 81.311 2.32217 81.5122 2.32217V2.32217ZM64.6115 2.01447C64.6115 1.9865 64.6115 1.9865 64.6115 2.01447V2.01447ZM55.1552 2.54594C54.954 2.54594 54.954 2.54594 54.7528 2.51797C54.7528 2.51797 54.5516 2.51797 54.5516 2.54594C54.7528 2.54594 54.954 2.54594 55.1552 2.54594ZM32.8222 2.18231C33.0234 2.15433 32.8222 2.15433 32.8222 2.18231V2.18231ZM65.4163 2.79769C65.6175 2.79769 65.6175 2.79769 65.4163 2.79769V2.79769ZM61.3923 2.71378C61.1911 2.71378 61.1911 2.74175 60.7887 2.74175C60.9899 2.71378 61.1911 2.71378 61.3923 2.71378ZM23.1647 2.18231C23.1647 2.21028 23.1647 2.21028 23.1647 2.18231V2.18231ZM148.109 1.62287C148.109 1.62287 148.109 1.59489 147.908 1.62287C147.908 1.62287 147.908 1.62287 147.706 1.62287C147.908 1.62287 147.908 1.62287 148.109 1.62287ZM164.406 2.62986C164.406 2.60189 164.607 2.60189 164.607 2.57392C164.205 2.60189 164.205 2.60189 164.406 2.62986ZM117.325 3.02147C117.325 3.02147 117.124 3.02147 117.124 3.04944C117.325 3.04944 117.728 3.04944 117.929 3.04944C117.728 3.02147 117.527 3.02147 117.325 3.02147ZM130.001 1.45503C130.001 1.42706 130.001 1.42706 130.001 1.45503C130.001 1.42706 129.8 1.42706 130.001 1.45503ZM160.583 1.84664C160.784 1.84664 160.784 1.84664 160.583 1.84664C160.784 1.81867 160.986 1.81867 160.986 1.7907C160.784 1.7907 160.583 1.7907 160.382 1.7907C160.583 1.81867 160.583 1.81867 160.583 1.84664ZM158.37 1.37112C158.169 1.39909 158.169 1.39909 158.37 1.37112V1.37112ZM100.224 3.41308C100.224 3.44105 100.224 3.44105 100.022 3.46902C100.224 3.46902 100.425 3.46902 100.626 3.46902C100.425 3.44105 100.425 3.44105 100.224 3.41308ZM141.268 0.867621C141.469 0.867621 141.469 0.867621 141.67 0.867621C141.67 0.867621 141.469 0.867621 141.268 0.867621ZM82.317 0.531957C82.1158 0.531957 82.1158 0.531957 82.317 0.531957V0.531957ZM190.562 3.58091C191.366 3.52496 191.97 3.55294 192.775 3.58091C192.171 3.58091 191.366 3.58091 190.562 3.58091ZM107.668 0.336153C107.869 0.336153 108.07 0.336153 108.272 0.364125C107.869 0.336153 107.668 0.336153 107.668 0.336153Z"
                                                fill="url(#paint0_linear_229_1115)" />
                                            <defs>
                                                <linearGradient id="paint0_linear_229_1115" x1="166.491" y1="0.000492651"
                                                    x2="10.1151" y2="73.2993" gradientUnits="userSpaceOnUse">
                                                    <stop offset="0" stop-color="#EF5C92" />
                                                    <stop offset="0.247843" stop-color="#A85CC5" />
                                                    <stop offset="1" stop-color="#6698FB" />
                                                </linearGradient>
                                            </defs>
                                        </svg>

                                    </span>
                                </span>
                                Company Limited.
                            </h3>
                        </div>
                        <div class="tp-service-2__text mb-25">
                            <p style="text-align: justify; color: black;">
                                Shwe Wutt Hmone Kha Co., Ltd (Overseas Myanmar Manpower Employment Agency) was founded in
                                2018, emerging as a distinguished and expertly managed international manpower recruitment
                                consultant. With a core focus on Thailand, Malaysia, and Japan job markets, we deliver
                                premium recruitment services to diverse industries. Drawing from our extensive recruitment
                                experience, we cater to various employment levels as per our clients' demands across
                                multiple sectors. These include poultry processing, food and beverages production, wood
                                processing, plastics and rubber manufacturing, leather production, construction, hospitality
                                services, agriculture, and other vital sectors.
                            </p>
                        </div>
                        <div class="tp-service-2__list mb-35">
                            <ul>
                                <li>
                                    <div class="tp-service-2__list-content d-flex align-items-center">
                                        <span>
                                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                                xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M19.7489 4.82063L10.2034 0.0478977C10.0755 -0.0159659 9.92455 -0.0159659 9.79682 0.0478977L0.251364 4.82063C0.0972727 4.89767 0 5.05494 0 5.22722V14.7727C0 14.9449 0.0972727 15.1022 0.251136 15.1793L9.79659 19.952C9.86068 19.984 9.93023 19.9999 10 19.9999C10.0698 19.9999 10.1393 19.984 10.2034 19.952L19.7489 15.1793C19.9027 15.1022 20 14.9449 20 14.7727V5.22722C20 5.05494 19.9027 4.89767 19.7489 4.82063ZM10 0.962671L18.5291 5.22722L16.7045 6.13949L8.17568 1.87494L10 0.962671ZM13.0682 7.95767L4.53909 3.69313L7.15909 2.38313L15.6882 6.64767L13.0682 7.95767ZM13.4091 8.80358L16.1364 7.43994V10.8211L15.2641 10.1981C15.1652 10.1274 15.0416 10.0995 14.9218 10.1202C14.802 10.1413 14.6955 10.2093 14.6261 10.3093L13.4091 12.0677V8.80358ZM3.52273 4.20131L12.0516 8.46585L10 9.49176L1.47091 5.22722L3.52273 4.20131ZM0.909091 5.96267L9.54545 10.2809V18.8097L0.909091 14.4915V5.96267ZM10.4545 18.8099V10.2809L12.5 9.25813V13.5227C12.5 13.7215 12.6291 13.8972 12.8189 13.9565C12.8639 13.9706 12.9093 13.9772 12.9545 13.9772C13.1009 13.9772 13.2418 13.9063 13.3284 13.7815L15.1114 11.2063L16.327 12.0745C16.4657 12.1734 16.648 12.1868 16.7991 12.1086C16.9502 12.0309 17.0455 11.8747 17.0455 11.7045V6.9854L19.0909 5.96267V14.4915L10.4545 18.8099Z"
                                                    fill="white" />
                                            </svg>
                                        </span>
                                        <h5 class="tp-service-2__title-sm">
                                            “Empowering Futures, Connecting Worlds”
                                        </h5>
                                    </div>
                                </li>
                            </ul>
                        </div>

                        <div class="row">
                            <div class="col-xl-6 col-12">
                                <div class="tp-service-2__list-box-2">
                                    <ul>
                                        <li>
                                            <i>
                                                <svg width="17" height="16" viewBox="0 0 17 16" fill="none"
                                                    xmlns="http://www.w3.org/2000/svg">
                                                    <path
                                                        d="M8.00832 0.000488281C3.59271 0.000488281 0 3.58946 0 8.00049C0 12.4115 3.59271 16.0005 8.00832 16.0005C12.4239 16.0005 16.0166 12.4115 16.0166 8.00049C16.0166 3.58946 12.4239 0.000488281 8.00832 0.000488281ZM12.4841 5.89523L7.36605 10.9679C7.06499 11.2687 6.58328 11.2887 6.26215 10.988L3.55256 8.52179C3.23143 8.22104 3.21136 7.71979 3.49235 7.39899C3.79342 7.07818 4.29519 7.05813 4.61633 7.35889L6.76392 9.3238L11.3401 4.75237C11.6612 4.43157 12.163 4.43157 12.4841 4.75237C12.8053 5.07317 12.8053 5.57442 12.4841 5.89523Z"
                                                        fill="#7252F3" />
                                                </svg>
                                            </i>
                                            Best Integrity Support
                                        </li>

                                        <li>
                                            <i>
                                                <svg width="17" height="16" viewBox="0 0 17 16" fill="none"
                                                    xmlns="http://www.w3.org/2000/svg">
                                                    <path
                                                        d="M8.00832 0.000488281C3.59271 0.000488281 0 3.58946 0 8.00049C0 12.4115 3.59271 16.0005 8.00832 16.0005C12.4239 16.0005 16.0166 12.4115 16.0166 8.00049C16.0166 3.58946 12.4239 0.000488281 8.00832 0.000488281ZM12.4841 5.89523L7.36605 10.9679C7.06499 11.2687 6.58328 11.2887 6.26215 10.988L3.55256 8.52179C3.23143 8.22104 3.21136 7.71979 3.49235 7.39899C3.79342 7.07818 4.29519 7.05813 4.61633 7.35889L6.76392 9.3238L11.3401 4.75237C11.6612 4.43157 12.163 4.43157 12.4841 4.75237C12.8053 5.07317 12.8053 5.57442 12.4841 5.89523Z"
                                                        fill="#7252F3" />
                                                </svg>
                                            </i>
                                            Respectful
                                        </li>

                                        <li>
                                            <i>
                                                <svg width="17" height="16" viewBox="0 0 17 16" fill="none"
                                                    xmlns="http://www.w3.org/2000/svg">
                                                    <path
                                                        d="M8.00832 0.000488281C3.59271 0.000488281 0 3.58946 0 8.00049C0 12.4115 3.59271 16.0005 8.00832 16.0005C12.4239 16.0005 16.0166 12.4115 16.0166 8.00049C16.0166 3.58946 12.4239 0.000488281 8.00832 0.000488281ZM12.4841 5.89523L7.36605 10.9679C7.06499 11.2687 6.58328 11.2887 6.26215 10.988L3.55256 8.52179C3.23143 8.22104 3.21136 7.71979 3.49235 7.39899C3.79342 7.07818 4.29519 7.05813 4.61633 7.35889L6.76392 9.3238L11.3401 4.75237C11.6612 4.43157 12.163 4.43157 12.4841 4.75237C12.8053 5.07317 12.8053 5.57442 12.4841 5.89523Z"
                                                        fill="#7252F3" />
                                                </svg>
                                            </i>
                                            Professional Expertise
                                        </li>

                                        <li>
                                            <i>
                                                <svg width="17" height="16" viewBox="0 0 17 16" fill="none"
                                                    xmlns="http://www.w3.org/2000/svg">
                                                    <path
                                                        d="M8.00832 0.000488281C3.59271 0.000488281 0 3.58946 0 8.00049C0 12.4115 3.59271 16.0005 8.00832 16.0005C12.4239 16.0005 16.0166 12.4115 16.0166 8.00049C16.0166 3.58946 12.4239 0.000488281 8.00832 0.000488281ZM12.4841 5.89523L7.36605 10.9679C7.06499 11.2687 6.58328 11.2887 6.26215 10.988L3.55256 8.52179C3.23143 8.22104 3.21136 7.71979 3.49235 7.39899C3.79342 7.07818 4.29519 7.05813 4.61633 7.35889L6.76392 9.3238L11.3401 4.75237C11.6612 4.43157 12.163 4.43157 12.4841 4.75237C12.8053 5.07317 12.8053 5.57442 12.4841 5.89523Z"
                                                        fill="#7252F3" />
                                                </svg>
                                            </i>
                                            Customer-Centricity
                                        </li>
                                    </ul>
                                </div>
                            </div>

                            <div class="col-xl-6 d-none d-xl-block">
                                <div class="tp-service-2__big-text p-relative">
                                    <span>
                                        <strong>
                                            5+
                                        </strong>
                                        Years Experience
                                    </span>
                                    <div class="tp-service-2__text-shape-1">
                                        <img src="{{ asset('assets/img/service/shape-3-3.jpg') }}" alt="">
                                    </div>
                                    <div class="tp-service-2__text-shape-2">
                                        <span>
                                            <svg width="10" height="10" viewBox="0 0 10 10" fill="none"
                                                xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M9.23 4.13449V5.69449H5.85V9.59449H4.03V5.69449H0.65V4.13449H4.03V0.234488H5.85V4.13449H9.23Z"
                                                    fill="#336EF9" />
                                            </svg>
                                        </span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>




    <div class="tp-catagori__area p-relative pb-115 pt-50 fix">
        <div class="tp-catagori__shape-1 d-none d-xxl-block">
            <img src="{{ asset('assets/img/catagori/shape-1-1.png') }}" alt="">
        </div>
        <div class="tp-catagori__shape-2 d-none d-xxl-block">
            <img src="{{ asset('assets/img/catagori/shape-1-2.png') }}" alt="">
        </div>
        <div class="container">
            <div class="row">
                <div class="col-xl-12">
                    <div class="tp-catagori__section-box text-center pb-55">
                        <span class="tp-section-subtitle">
                            Shwe Wutt Hmone Kha Co.,Ltd
                        </span>
                        <h3 class="tp-section-title">
                            Why Choose
                            <span class="p-relative">
                                Us
                                <span class="tp-title-shape">
                                    <svg width="244" height="4" viewBox="0 0 244 4" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M240.76 3.21678C241.508 3.21678 242.754 3.18881 243.502 3.24476C242.256 3.24476 241.01 3.27273 240.013 3.27273C239.514 3.32867 240.76 3.35664 241.259 3.38462C242.256 3.38462 243.252 3.38462 244 3.44056C243.252 3.46853 242.256 3.46853 241.259 3.46853C240.262 3.46853 239.016 3.44056 238.268 3.4965C239.016 3.52448 239.016 3.52448 239.016 3.58042C237.272 3.60839 237.272 3.77622 234.53 3.72028C233.534 3.72028 232.537 3.72028 231.54 3.72028C230.294 3.83217 228.799 3.94406 225.559 3.88811C224.562 3.91608 223.566 3.91608 222.569 3.88811C219.828 3.94406 217.086 3.91608 214.345 3.94406C214.096 3.97203 213.847 3.97203 213.598 3.94406C212.601 3.94406 211.604 3.94406 210.607 3.94406H210.358H210.109C209.112 3.94406 208.364 3.97203 207.368 3.94406C203.879 3.94406 200.141 3.94406 196.652 3.97203C194.409 3.97203 192.665 3.94406 191.419 3.86014H191.17H190.92C190.173 3.91608 190.422 4 187.93 4C185.438 3.97203 182.946 3.97203 180.205 3.97203C178.46 3.94406 176.716 4 174.972 3.97203C168.742 3.94406 162.512 3.94406 156.282 3.91608C150.799 3.91608 145.317 3.88811 139.834 3.86014C137.841 3.91608 135.847 3.86014 134.103 3.86014C124.135 3.83217 113.918 3.83217 103.95 3.74825C99.464 3.72028 94.9784 3.74825 90.4928 3.69231C86.5056 3.69231 82.7676 3.66434 78.7804 3.63636C69.56 3.58042 60.3396 3.52448 51.3684 3.41259C47.6304 3.41259 44.1416 3.38462 40.9021 3.3007C35.6689 3.3007 30.6849 3.21678 25.9501 3.13287C25.2025 3.13287 24.7041 3.1049 24.2057 3.1049C22.2121 3.13287 20.7169 3.07692 18.9725 3.02098C17.7265 3.02098 16.7297 2.99301 15.9821 2.96504C15.2345 2.93706 14.4869 2.93706 13.7393 2.90909C12.9917 2.85315 11.7457 2.85315 10.4997 2.85315C8.50606 2.7972 6.01407 2.76923 5.51567 2.62937C1.77766 2.6014 0.780869 2.48951 1.03006 2.34965C-0.465134 2.18182 -0.215942 2.01399 1.03006 1.84615C1.03006 1.76224 1.03006 1.67832 1.03006 1.59441V1.56643C0.531662 1.37063 3.27286 1.2028 2.52525 1.03497C2.27606 0.895105 3.77124 0.783217 4.76805 0.671329C4.76805 0.643357 4.76807 0.587413 5.76486 0.559441C6.01405 0.531469 6.76167 0.503497 7.75847 0.475525C9.00447 0.363636 10.4996 0.27972 12.9916 0.251748C13.49 0.195804 14.4868 0.223776 15.2344 0.223776C15.982 0.195804 16.7296 0.167832 17.4772 0.13986C18.2248 0.111888 18.9724 0.111888 19.72 0.111888C22.7104 0.0559441 25.7009 0.027972 28.9404 0.027972C30.1864 0.027972 31.1833 0.027972 32.4293 0.027972C32.9277 0.027972 33.6752 0.027972 34.1736 0C40.902 0 47.8797 0 54.6081 0.027972C58.0969 0.027972 61.5856 0 65.0744 0.0839161C68.0648 0.0839161 71.5536 0.027972 74.7932 0.111888C75.5408 0.111888 76.2884 0.111888 76.7868 0.111888C79.0296 0.0839161 81.0233 0.0839161 83.2661 0.111888C88.7485 0.111888 94.2308 0.13986 99.464 0.167832C101.208 0.167832 103.202 0.13986 104.697 0.195804C105.694 0.223776 106.691 0.223776 107.688 0.195804C110.927 0.195804 114.167 0.195804 117.406 0.223776C117.905 0.223776 118.403 0.223776 118.902 0.223776C120.148 0.251748 121.643 0.223776 122.889 0.251748C124.633 0.251748 126.627 0.251748 128.371 0.251748C129.866 0.251748 131.611 0.251748 132.857 0.27972C133.854 0.27972 134.85 0.27972 135.847 0.27972C137.841 0.27972 140.084 0.251748 142.077 0.307692C145.815 0.307692 149.553 0.335664 153.042 0.335664C159.023 0.335664 165.004 0.391608 170.984 0.41958C172.978 0.41958 174.722 0.41958 176.716 0.447553C174.972 0.447553 172.978 0.447553 171.234 0.447553C172.978 0.41958 174.972 0.41958 176.716 0.447553C179.956 0.447553 182.946 0.447553 185.936 0.503497C187.182 0.503497 188.428 0.503497 189.674 0.531469C189.924 0.531469 190.173 0.531469 190.422 0.531469C193.163 0.531469 195.904 0.559441 198.646 0.615385C201.138 0.615385 203.38 0.671329 205.623 0.699301C206.869 0.699301 208.115 0.699301 209.361 0.755245C211.604 0.783217 213.598 0.811189 214.345 0.895105C215.84 0.839161 216.837 0.895105 218.083 0.923077C219.329 0.979021 218.831 1.06294 220.326 1.11888C219.578 1.11888 218.831 1.11888 218.083 1.11888C218.332 1.14685 218.582 1.17483 218.831 1.17483C220.575 1.23077 222.32 1.25874 224.064 1.28671C224.313 1.28671 224.562 1.28671 224.812 1.28671C226.556 1.28671 228.55 1.25874 230.045 1.31469H230.294C230.294 1.31469 230.294 1.34266 230.045 1.34266C229.048 1.34266 228.051 1.34266 227.304 1.34266H227.054C228.3 1.31469 229.297 1.31469 230.045 1.34266C230.543 1.45455 229.048 1.48252 228.55 1.53846C228.799 1.56643 227.553 1.59441 228.55 1.62238C229.297 1.62238 230.543 1.59441 230.294 1.65035C230.045 1.70629 228.55 1.67832 227.802 1.67832C227.553 1.67832 227.304 1.70629 227.054 1.70629C229.048 1.79021 231.042 1.81818 233.035 1.87413C236.026 1.87413 237.272 1.98601 239.016 2.06993C239.514 2.0979 240.262 2.06993 240.511 2.0979C239.016 2.0979 239.764 2.18182 238.767 2.20979C235.278 2.26573 235.278 2.34965 238.518 2.46154C239.514 2.48951 241.508 2.51748 242.006 2.6014C242.006 2.62937 242.006 2.65734 242.006 2.65734C242.505 2.65734 243.003 2.65734 243.003 2.68531C242.754 2.74126 242.006 2.71329 241.259 2.71329C240.76 2.71329 240.013 2.71329 239.764 2.74126C240.262 2.82518 240.013 2.85315 238.518 2.85315C239.016 2.90909 240.511 2.88112 240.262 2.96504C241.01 2.96504 241.508 2.99301 242.256 2.99301C241.508 3.04895 240.013 2.99301 239.764 3.04895C240.511 3.1049 240.511 3.13287 239.265 3.1049C238.268 3.1049 237.022 3.07692 238.268 3.16084C239.016 3.16084 240.013 3.16084 240.76 3.21678ZM7.01083 1.48252C6.76163 1.48252 6.76164 1.48252 6.76164 1.51049C6.76164 1.48252 7.01083 1.48252 7.01083 1.48252ZM69.8092 1.9021C70.0585 1.93007 70.0584 1.9021 70.3076 1.9021C70.0584 1.9021 69.8092 1.9021 69.8092 1.9021ZM100.959 2.32168C100.71 2.32168 100.71 2.32168 100.959 2.32168V2.32168ZM80.0264 2.01399C80.0264 1.98601 80.0264 1.98601 80.0264 2.01399V2.01399ZM68.314 2.54545C68.0648 2.54545 68.0648 2.54545 67.8156 2.51748C67.8156 2.51748 67.5665 2.51748 67.5665 2.54545C67.8156 2.54545 68.0648 2.54545 68.314 2.54545ZM40.6528 2.18182C40.902 2.15385 40.6528 2.15385 40.6528 2.18182V2.18182ZM81.0232 2.7972C81.2724 2.7972 81.2724 2.7972 81.0232 2.7972V2.7972ZM76.0392 2.71329C75.79 2.71329 75.79 2.74126 75.2916 2.74126C75.5408 2.71329 75.79 2.71329 76.0392 2.71329ZM28.6912 2.18182C28.6912 2.20979 28.6912 2.20979 28.6912 2.18182V2.18182ZM183.444 1.62238C183.444 1.62238 183.444 1.59441 183.195 1.62238C183.195 1.62238 183.195 1.62238 182.946 1.62238C183.195 1.62238 183.195 1.62238 183.444 1.62238ZM203.63 2.62937C203.63 2.6014 203.879 2.6014 203.879 2.57343C203.38 2.6014 203.38 2.6014 203.63 2.62937ZM145.317 3.02098C145.317 3.02098 145.068 3.02098 145.068 3.04895C145.317 3.04895 145.815 3.04895 146.064 3.04895C145.815 3.02098 145.566 3.02098 145.317 3.02098ZM161.016 1.45455C161.016 1.42657 161.016 1.42657 161.016 1.45455C161.016 1.42657 160.767 1.42657 161.016 1.45455ZM198.895 1.84615C199.144 1.84615 199.144 1.84615 198.895 1.84615C199.144 1.81818 199.393 1.81818 199.393 1.79021C199.144 1.79021 198.895 1.79021 198.646 1.79021C198.895 1.81818 198.895 1.81818 198.895 1.84615ZM196.154 1.37063C195.904 1.3986 195.904 1.3986 196.154 1.37063V1.37063ZM124.135 3.41259C124.135 3.44056 124.135 3.44056 123.886 3.46853C124.135 3.46853 124.384 3.46853 124.633 3.46853C124.384 3.44056 124.384 3.44056 124.135 3.41259ZM174.972 0.867133C175.221 0.867133 175.221 0.867133 175.47 0.867133C175.47 0.867133 175.221 0.867133 174.972 0.867133ZM101.956 0.531469C101.707 0.531469 101.707 0.531469 101.956 0.531469V0.531469ZM236.026 3.58042C237.022 3.52448 237.77 3.55245 238.767 3.58042C238.019 3.58042 237.022 3.58042 236.026 3.58042ZM133.355 0.335664C133.604 0.335664 133.854 0.335664 134.103 0.363636C133.604 0.335664 133.355 0.335664 133.355 0.335664Z"
                                            fill="url(#paint0_linear_74_17647)" />
                                        <defs>
                                            <linearGradient id="paint0_linear_74_17647" x1="206.212" y1="4.37008e-06"
                                                x2="29.5265" y2="102.578" gradientUnits="userSpaceOnUse">
                                                <stop offset="0" stop-color="#EF5C92" />
                                                <stop offset="0.247843" stop-color="#A85CC5" />
                                                <stop offset="1" stop-color="#6698FB" />
                                            </linearGradient>
                                        </defs>
                                    </svg>
                                </span>
                            </span>
                        </h3>
                    </div>
                </div>
            </div>

            <div class="tp-catagori__box p-relative">
                <div class="tp-catagori__wrapper">
                    <div class="swiper-container tp-catagori__active">
                        <div class="swiper-wrapper">
                            <div class="swiper-slide">
                                <div class="tp-catagori__catagori-wrap">
                                    <div class="tp-catagori__catagori-box">
                                        <div class="row g-0">

                                            <div class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-catagori-1">
                                                <div class="tp-catagori__border">
                                                    <div class="tp-catagori__item p-relative d-flex align-items-center">
                                                        <div class="tp-catagori__item-text">
                                                            <span>
                                                                SWHK
                                                            </span>
                                                        </div>
                                                        <div class="tp-catagori__thumb">
                                                            <img src="{{ asset('data/icon/partners.png') }}"
                                                                alt=""
                                                                style="width: 150px; background-color: #92cc95; border-radius: 50%;">
                                                            <div class="tp-catagori__shape">
                                                                <img src="{{ asset('assets/img/catagori/icon-shape-1.png') }}"
                                                                    alt="">
                                                            </div>
                                                        </div>
                                                        <div class="tp-catagori__text">
                                                            <h5 class="tp-catagori__title">
                                                                <a href="javascript::void(0)">
                                                                    Expert Teams
                                                                </a>
                                                            </h5>
                                                            <p>
                                                                Highly skilled and knowledgeable professionals.
                                                            </p>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>

                                            <div class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-catagori-2">
                                                <div class="tp-catagori__border">
                                                    <div class="tp-catagori__item p-relative d-flex align-items-center">
                                                        <div class="tp-catagori__item-text">
                                                            <span>
                                                                SWHK
                                                            </span>
                                                        </div>
                                                        <div class="tp-catagori__thumb">
                                                            <img src="{{ asset('data/icon/etiquette.png') }}"
                                                                alt=""
                                                                style="width: 150px; background-color: #c9d4db; border-radius: 50%;">
                                                            <div class="tp-catagori__shape">
                                                                <img src="{{ asset('assets/img/catagori/icon-shape-1.png') }}"
                                                                    alt="">
                                                            </div>
                                                        </div>
                                                        <div class="tp-catagori__text">
                                                            <h5 class="tp-catagori__title">
                                                                <a href="javascript::void(0)">
                                                                    Commitment to Clients
                                                                </a>
                                                            </h5>
                                                            <p>
                                                                Prioritizing client satisfaction and needs.
                                                            </p>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>

                                            <div class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-catagori-3">
                                                <div class="tp-catagori__border">
                                                    <div
                                                        class="tp-catagori__item tp-catagori__item-2 p-relative d-flex align-items-center">
                                                        <div class="tp-catagori__item-text">
                                                            <span>
                                                                SWHK
                                                            </span>
                                                        </div>
                                                        <div class="tp-catagori__thumb">
                                                            <img src="{{ asset('data/icon/innovation.png') }}"
                                                                alt=""
                                                                style="width: 150px; background-color: #c9d4db; border-radius: 50%;">
                                                            <div class="tp-catagori__shape">
                                                                <img src="{{ asset('assets/img/catagori/icon-shape-1.png') }}"
                                                                    alt="">
                                                            </div>
                                                        </div>
                                                        <div class="tp-catagori__text">
                                                            <h5 class="tp-catagori__title">
                                                                <a href="javascript::void(0)">
                                                                    Innovation
                                                                </a>
                                                            </h5>
                                                            <p>
                                                                Embracing new solutions and staying at the cutting edge.
                                                            </p>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>

                                            <div class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-catagori-4">
                                                <div class="tp-catagori__border">
                                                    <div
                                                        class="tp-catagori__item tp-catagori__item-2 p-relative d-flex align-items-center">
                                                        <div class="tp-catagori__item-text">
                                                            <span>
                                                                SWHK
                                                            </span>
                                                        </div>
                                                        <div class="tp-catagori__thumb">
                                                            <img src="{{ asset('data/icon/reputation-management.png') }}"
                                                                alt=""
                                                                style="width: 150px; background-color: #c9d4db; border-radius: 50%;">
                                                            <div class="tp-catagori__shape">
                                                                <img src="{{ asset('assets/img/catagori/icon-shape-1.png') }}"
                                                                    alt="">
                                                            </div>
                                                        </div>
                                                        <div class="tp-catagori__text">
                                                            <h5 class="tp-catagori__title">
                                                                <a href="javascript::void(0)">
                                                                    Reputation
                                                                </a>
                                                            </h5>
                                                            <p>
                                                                Known for reliability and excellence in the industry.
                                                            </p>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-xl-12">
                <div class="tp-catagori__button text-center mt-70">
                    <a class="tp-btn-gradiant" href="{{ route('contact.index') }}">
                        Let’s talk — Send a message
                        <i>
                            <svg width="18" height="17" viewBox="0 0 18 17" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M17.6742 4.74002C17.3484 4.25133 16.8597 3.92554 16.2896 3.76264C15.0679 3.5183 13.9276 4.25133 13.6018 5.3916L12.8688 7.99794L5.78281 1.07486C4.96833 0.260384 3.66516 0.260384 2.93213 1.07486L2.85068 1.15631C2.60633 1.40065 2.52489 1.645 2.44344 1.97079C1.8733 1.88934 1.22172 2.13369 0.81448 2.54093C-0.081448 3.27396 -0.162896 4.57712 0.570136 5.3916L0.651584 5.47305L1.14027 6.04319C0.977376 6.12463 0.81448 6.20608 0.651584 6.36898C0.244344 6.77622 0 7.26491 0 7.83504C0 8.40518 0.244344 8.89386 0.570136 9.38255C5.04977 13.9436 6.67873 15.4097 7.08597 15.7355C7.9819 16.4685 9.36652 16.9572 10.6697 16.9572C11.4842 16.9572 12.2172 16.7943 12.9502 16.3056C14.3348 15.3282 15.4751 14.0251 16.0452 12.8034C16.2896 12.3961 16.6968 11.1744 17.9186 6.36898C18.0814 5.79884 18 5.22871 17.6742 4.74002ZM14.4163 12.2332C14.0091 13.2106 13.0317 14.2694 11.8914 15.0025C11.0769 15.5726 9.12217 15.2468 8.1448 14.4323C7.819 14.1065 6.19005 12.7219 1.79186 8.16083C1.71041 8.07938 1.62896 7.99794 1.62896 7.83504C1.62896 7.75359 1.71041 7.5907 1.79186 7.50925C1.95475 7.26491 2.28054 7.26491 2.52489 7.50925L5.04977 10.0341C5.21267 10.197 5.45701 10.2785 5.61991 10.2785H5.70136C5.86425 10.2785 6.1086 10.197 6.27149 9.95269C6.59728 9.62689 6.59728 9.13821 6.27149 8.81242L1.79186 4.25133C1.62896 4.08844 1.62896 3.84409 1.79186 3.76264C2.0362 3.5183 2.36199 3.5183 2.60633 3.76264L4.80543 5.88029L7.00453 8.07938C7.16742 8.24228 7.41176 8.32373 7.65611 8.32373C7.90045 8.32373 8.06335 8.24228 8.22624 8.07938C8.55204 7.75359 8.55204 7.26491 8.22624 6.93911L4.0724 2.86672C3.9095 2.70382 3.9095 2.37803 4.0724 2.13369C4.23529 1.97079 4.47964 1.97079 4.64253 2.13369L12.7059 10.0341C12.8688 10.197 13.0317 10.2785 13.276 10.2785C13.5204 10.2785 13.7647 10.197 13.8462 10.1156C14.0091 9.95269 14.009 9.87124 14.0905 9.70834V9.62689L15.1493 5.79884C15.2308 5.47305 15.5566 5.31016 15.8824 5.3916C16.0452 5.3916 16.1267 5.47305 16.2081 5.63595C16.2896 5.79884 16.2896 5.88029 16.2896 6.04319C15.1493 10.3599 14.6606 11.826 14.4163 12.2332ZM8.06335 1.56355C8.30769 1.645 8.47059 1.80789 8.71493 1.88934C8.95928 1.97079 9.12217 2.13369 9.36652 2.29658C9.69231 2.54093 10.0995 2.86672 10.4253 3.19251C10.7511 3.5183 11.0769 3.84409 11.3213 4.25133C11.4842 4.41423 11.7285 4.57712 11.9729 4.57712C12.1357 4.57712 12.2986 4.49568 12.3801 4.41423C12.7873 4.16988 12.8688 3.6812 12.6244 3.27396C12.2986 2.78527 11.9729 2.37803 11.5656 1.97079C11.1584 1.56355 10.7511 1.23776 10.2624 0.911967C10.0181 0.749072 9.77376 0.667624 9.52941 0.504728C9.20362 0.341832 8.95928 0.178936 8.71493 0.0974882C8.30769 -0.146856 7.819 0.0974882 7.65611 0.504728C7.41176 0.911967 7.65611 1.40065 8.06335 1.56355ZM4.96833 14.7581C4.72398 14.6767 4.47964 14.5138 4.31674 14.4323C4.0724 14.2694 3.9095 14.188 3.66516 14.0251C3.25792 13.6993 2.93213 13.3735 2.60633 13.0477C2.28054 12.7219 2.0362 12.3147 1.79186 11.9074C1.54751 11.5816 1.05882 11.4187 0.651584 11.6631C0.325792 11.9074 0.162896 12.3961 0.40724 12.8034C0.733032 13.2921 1.05882 13.7807 1.46606 14.188C1.8733 14.5952 2.28054 15.0025 2.76923 15.3282L3.50226 15.8169C3.50226 15.8169 3.99095 16.0613 4.23529 16.2242C4.31674 16.3056 4.47964 16.3056 4.56109 16.3056C4.88688 16.3056 5.13122 16.1427 5.37557 15.8169C5.53846 15.4097 5.37557 14.921 4.96833 14.7581Z"
                                    fill="white" />
                            </svg>
                        </i>
                    </a>
                </div>
            </div>
        </div>
    </div>


    <div class="tp-cta-4__area fix">
        <div class="tp-cta-4__bg p-relative">

            <div class="tp-cta-4__shape-2 d-none d-xl-block" style="right: -0%;">
                <img src="{{ asset('data/goal.png') }}" alt="" style="width: 30%; float: right;">
            </div>

            <div class="tp-cta-4__shape-3 d-none d-lg-block">
                <img src="{{ asset('assets/img/cta/cta-4-3.png') }}" alt="">
            </div>

            <div class="tp-cta-4__shape-4 d-none d-lg-block">
                <img src="{{ asset('assets/img/cta/cta-4-4.png') }}" alt="">
            </div>

            <div class="tp-cta-4__shape-5">
                <img src="{{ asset('assets/img/cta/cta-shape-4-1.png') }}" alt="">
            </div>

            <div class="container custom-container-3">
                <div class="row">
                    <div class="col-xl-12">
                        <div class="tp-cta-4__content text-center z-index">
                            <h3 class="tp-cta-4__title">
                                Our Objectives
                                <br>
                                <span class="p-relative">
                                    <span class="tp-title-shape">
                                        <svg width="147" height="4" viewBox="0 0 147 4" fill="none"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
                                                d="M145.048 3.21678C145.499 3.21678 146.249 3.18881 146.7 3.24476C145.949 3.24476 145.198 3.27273 144.598 3.27273C144.298 3.32867 145.048 3.35664 145.349 3.38462C145.949 3.38462 146.55 3.38462 147 3.44056C146.55 3.46853 145.949 3.46853 145.349 3.46853C144.748 3.46853 143.997 3.44056 143.547 3.4965C143.997 3.52448 143.997 3.52448 143.997 3.58042C142.946 3.60839 142.946 3.77622 141.295 3.72028C140.694 3.72028 140.094 3.72028 139.493 3.72028C138.743 3.83217 137.842 3.94406 135.89 3.88811C135.29 3.91608 134.689 3.91608 134.089 3.88811C132.437 3.94406 130.786 3.91608 129.134 3.94406C128.984 3.97203 128.834 3.97203 128.684 3.94406C128.083 3.94406 127.483 3.94406 126.882 3.94406H126.732H126.582C125.981 3.94406 125.531 3.97203 124.93 3.94406C122.829 3.94406 120.577 3.94406 118.475 3.97203C117.124 3.97203 116.073 3.94406 115.322 3.86014H115.172H115.022C114.571 3.91608 114.721 4 113.22 4C111.719 3.97203 110.217 3.97203 108.566 3.97203C107.515 3.94406 106.464 4 105.413 3.97203C101.66 3.94406 97.9066 3.94406 94.1533 3.91608C90.8504 3.91608 87.5474 3.88811 84.2445 3.86014C83.0434 3.91608 81.8424 3.86014 80.7915 3.86014C74.7861 3.83217 68.6307 3.83217 62.6254 3.74825C59.923 3.72028 57.2206 3.74825 54.5182 3.69231C52.1161 3.69231 49.8641 3.66434 47.462 3.63636C41.9071 3.58042 36.3522 3.52448 30.9474 3.41259C28.6954 3.41259 26.5935 3.38462 24.6418 3.3007C21.489 3.3007 18.4864 3.21678 15.6339 3.13287C15.1834 3.13287 14.8832 3.1049 14.5829 3.1049C13.3819 3.13287 12.4811 3.07692 11.4301 3.02098C10.6795 3.02098 10.0789 2.99301 9.62854 2.96504C9.17813 2.93706 8.72774 2.93706 8.27734 2.90909C7.82693 2.85315 7.07628 2.85315 6.32561 2.85315C5.12454 2.7972 3.62323 2.76923 3.32297 2.62937C1.07097 2.6014 0.470444 2.48951 0.620575 2.34965C-0.280228 2.18182 -0.130096 2.01399 0.620575 1.84615C0.620575 1.76224 0.620575 1.67832 0.620575 1.59441V1.56643C0.320297 1.37063 1.97176 1.2028 1.52136 1.03497C1.37123 0.895105 2.27202 0.783217 2.87254 0.671329C2.87254 0.643357 2.87256 0.587413 3.47308 0.559441C3.62323 0.531469 4.07362 0.503497 4.67416 0.475525C5.42482 0.363636 6.32559 0.27972 7.82692 0.251748C8.1272 0.195804 8.72772 0.223776 9.17812 0.223776C9.62852 0.195804 10.0789 0.167832 10.5293 0.13986C10.9797 0.111888 11.4301 0.111888 11.8805 0.111888C13.6821 0.0559441 15.4837 0.027972 17.4354 0.027972C18.1861 0.027972 18.7866 0.027972 19.5373 0.027972C19.8376 0.027972 20.2879 0.027972 20.5882 0C24.6418 0 28.8455 0 32.8991 0.027972C35.001 0.027972 37.1028 0 39.2047 0.0839161C41.0063 0.0839161 43.1081 0.027972 45.0599 0.111888C45.5103 0.111888 45.9607 0.111888 46.2609 0.111888C47.6121 0.0839161 48.8132 0.0839161 50.1644 0.111888C53.4673 0.111888 56.7702 0.13986 59.923 0.167832C60.9739 0.167832 62.175 0.13986 63.0758 0.195804C63.6763 0.223776 64.2769 0.223776 64.8774 0.195804C66.8291 0.195804 68.7808 0.195804 70.7326 0.223776C71.0328 0.223776 71.3331 0.223776 71.6334 0.223776C72.384 0.251748 73.2848 0.223776 74.0355 0.251748C75.0864 0.251748 76.2875 0.251748 77.3384 0.251748C78.2392 0.251748 79.2901 0.251748 80.0408 0.27972C80.6413 0.27972 81.2419 0.27972 81.8424 0.27972C83.0434 0.27972 84.3946 0.251748 85.5957 0.307692C87.8477 0.307692 90.0997 0.335664 92.2015 0.335664C95.8047 0.335664 99.4079 0.391608 103.011 0.41958C104.212 0.41958 105.263 0.41958 106.464 0.447553C105.413 0.447553 104.212 0.447553 103.161 0.447553C104.212 0.41958 105.413 0.41958 106.464 0.447553C108.416 0.447553 110.217 0.447553 112.019 0.503497C112.77 0.503497 113.52 0.503497 114.271 0.531469C114.421 0.531469 114.571 0.531469 114.721 0.531469C116.373 0.531469 118.024 0.559441 119.676 0.615385C121.177 0.615385 122.528 0.671329 123.88 0.699301C124.63 0.699301 125.381 0.699301 126.132 0.755245C127.483 0.783217 128.684 0.811189 129.134 0.895105C130.035 0.839161 130.636 0.895105 131.386 0.923077C132.137 0.979021 131.837 1.06294 132.737 1.11888C132.287 1.11888 131.837 1.11888 131.386 1.11888C131.536 1.14685 131.686 1.17483 131.837 1.17483C132.888 1.23077 133.938 1.25874 134.989 1.28671C135.14 1.28671 135.29 1.28671 135.44 1.28671C136.491 1.28671 137.692 1.25874 138.593 1.31469H138.743C138.743 1.31469 138.743 1.34266 138.593 1.34266C137.992 1.34266 137.392 1.34266 136.941 1.34266H136.791C137.542 1.31469 138.142 1.31469 138.593 1.34266C138.893 1.45455 137.992 1.48252 137.692 1.53846C137.842 1.56643 137.091 1.59441 137.692 1.62238C138.142 1.62238 138.893 1.59441 138.743 1.65035C138.593 1.70629 137.692 1.67832 137.241 1.67832C137.091 1.67832 136.941 1.70629 136.791 1.70629C137.992 1.79021 139.193 1.81818 140.394 1.87413C142.196 1.87413 142.946 1.98601 143.997 2.06993C144.298 2.0979 144.748 2.06993 144.898 2.0979C143.997 2.0979 144.448 2.18182 143.847 2.20979C141.745 2.26573 141.745 2.34965 143.697 2.46154C144.298 2.48951 145.499 2.51748 145.799 2.6014C145.799 2.62937 145.799 2.65734 145.799 2.65734C146.099 2.65734 146.399 2.65734 146.399 2.68531C146.249 2.74126 145.799 2.71329 145.349 2.71329C145.048 2.71329 144.598 2.71329 144.448 2.74126C144.748 2.82518 144.598 2.85315 143.697 2.85315C143.997 2.90909 144.898 2.88112 144.748 2.96504C145.198 2.96504 145.499 2.99301 145.949 2.99301C145.499 3.04895 144.598 2.99301 144.448 3.04895C144.898 3.1049 144.898 3.13287 144.147 3.1049C143.547 3.1049 142.796 3.07692 143.547 3.16084C143.997 3.16084 144.598 3.16084 145.048 3.21678ZM4.22374 1.48252C4.07361 1.48252 4.07361 1.48252 4.07361 1.51049C4.07361 1.48252 4.22374 1.48252 4.22374 1.48252ZM42.0572 1.9021C42.2073 1.93007 42.2073 1.9021 42.3575 1.9021C42.2073 1.9021 42.0572 1.9021 42.0572 1.9021ZM60.8238 2.32168C60.6737 2.32168 60.6737 2.32168 60.8238 2.32168V2.32168ZM48.2126 2.01399C48.2126 1.98601 48.2126 1.98601 48.2126 2.01399V2.01399ZM41.1564 2.54545C41.0063 2.54545 41.0063 2.54545 40.8561 2.51748C40.8561 2.51748 40.706 2.51748 40.706 2.54545C40.8561 2.54545 41.0063 2.54545 41.1564 2.54545ZM24.4917 2.18182C24.6418 2.15385 24.4917 2.15385 24.4917 2.18182V2.18182ZM48.8132 2.7972C48.9633 2.7972 48.9633 2.7972 48.8132 2.7972V2.7972ZM45.8105 2.71329C45.6604 2.71329 45.6604 2.74126 45.3601 2.74126C45.5102 2.71329 45.6604 2.71329 45.8105 2.71329ZM17.2853 2.18182C17.2853 2.20979 17.2853 2.20979 17.2853 2.18182V2.18182ZM110.518 1.62238C110.518 1.62238 110.518 1.59441 110.368 1.62238C110.368 1.62238 110.368 1.62238 110.217 1.62238C110.368 1.62238 110.368 1.62238 110.518 1.62238ZM122.678 2.62937C122.678 2.6014 122.829 2.6014 122.829 2.57343C122.528 2.6014 122.528 2.6014 122.678 2.62937ZM87.5474 3.02098C87.5474 3.02098 87.3973 3.02098 87.3973 3.04895C87.5474 3.04895 87.8477 3.04895 87.9978 3.04895C87.8477 3.02098 87.6976 3.02098 87.5474 3.02098ZM97.0058 1.45455C97.0058 1.42657 97.0058 1.42657 97.0058 1.45455C97.0058 1.42657 96.8557 1.42657 97.0058 1.45455ZM119.826 1.84615C119.976 1.84615 119.976 1.84615 119.826 1.84615C119.976 1.81818 120.126 1.81818 120.126 1.79021C119.976 1.79021 119.826 1.79021 119.676 1.79021C119.826 1.81818 119.826 1.81818 119.826 1.84615ZM118.175 1.37063C118.024 1.3986 118.024 1.3986 118.175 1.37063V1.37063ZM74.7861 3.41259C74.7861 3.44056 74.7861 3.44056 74.636 3.46853C74.7861 3.46853 74.9363 3.46853 75.0864 3.46853C74.9363 3.44056 74.9363 3.44056 74.7861 3.41259ZM105.413 0.867133C105.563 0.867133 105.563 0.867133 105.713 0.867133C105.713 0.867133 105.563 0.867133 105.413 0.867133ZM61.4243 0.531469C61.2742 0.531469 61.2742 0.531469 61.4243 0.531469V0.531469ZM142.196 3.58042C142.796 3.52448 143.247 3.55245 143.847 3.58042C143.397 3.58042 142.796 3.58042 142.196 3.58042ZM80.3411 0.335664C80.4912 0.335664 80.6413 0.335664 80.7915 0.363636C80.4912 0.335664 80.3411 0.335664 80.3411 0.335664Z"
                                                fill="url(#paint0_linear_122_8)" />
                                            <defs>
                                                <linearGradient id="paint0_linear_122_8" x1="124.235" y1="4.37008e-06"
                                                    x2="-2.57609" y2="44.3541" gradientUnits="userSpaceOnUse">
                                                    <stop offset="0" stop-color="#EF5C92" />
                                                    <stop offset="0.247843" stop-color="#A85CC5" />
                                                    <stop offset="1" stop-color="#6698FB" />
                                                </linearGradient>
                                            </defs>
                                        </svg>

                                    </span>
                                </span>
                            </h3>
                            <p style="color: white; font-size: 20px;">
                                Our central objective is to bridge the gap between Myanmar's skilled workforce and the
                                global
                                business landscape. By sourcing and deploying highly skilled individuals, we aim to fulfill
                                the
                                employment needs of various industries while actively combating unemployment challenges
                                within our country.
                            </p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>



    <div class="tp-choose-6__area pt-50 pb-30">
        <div class="container">
            <div class="row">
                <div class="col-xl-12">
                    <div class="tp-choose-6__section-box text-center mb-50">
                        <span class="tp-gradiant-section-subtitle">
                            Shwe Wutt Hmone Kha Co.,Ltd
                        </span>
                        <h3 class="tp-section-title-3">Our Vision, <span class="p-relative">
                                Mission & Value
                                <span class="tp-title-shape">
                                    <svg width="233" height="5" viewBox="0 0 233 5" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M229.237 2.64045C229.948 2.63606 231.134 2.60076 231.845 2.6523C230.66 2.65963 229.474 2.69493 228.526 2.70079C228.052 2.75967 229.238 2.78031 229.712 2.80535C230.661 2.79949 231.609 2.79363 232.321 2.84517C231.61 2.87754 230.661 2.8834 229.713 2.88927C228.764 2.89513 227.579 2.87449 226.868 2.93483C227.579 2.9584 227.579 2.9584 227.579 3.01434C225.92 3.05258 225.921 3.2204 223.312 3.18058C222.364 3.18645 221.415 3.19231 220.467 3.19817C219.282 3.31739 217.86 3.43807 214.777 3.40118C213.829 3.43501 212.88 3.44088 211.932 3.41877C209.324 3.49083 206.715 3.47899 204.107 3.52308C203.87 3.55252 203.633 3.55398 203.396 3.52748C202.448 3.53334 201.499 3.5392 200.551 3.54507L200.314 3.54653L200.076 3.548C199.128 3.55386 198.417 3.58623 197.468 3.56412C194.149 3.58464 190.592 3.60663 187.272 3.65512C185.138 3.66831 183.478 3.6506 182.292 3.57401L182.055 3.57548L181.818 3.57694C181.107 3.63728 181.345 3.71973 178.974 3.73439C176.602 3.72108 174.231 3.73573 171.623 3.75186C169.963 3.73415 168.303 3.80035 166.644 3.78264C160.715 3.79131 154.788 3.82795 148.86 3.83662C143.643 3.86887 138.426 3.87314 133.21 3.87742C131.313 3.94509 129.416 3.90087 127.756 3.91113C118.271 3.94179 108.55 4.00188 99.0646 3.9766C94.7964 3.97501 90.5285 4.02936 86.2601 3.9998C82.4663 4.02326 78.9094 4.01727 75.1154 4.01275C66.3418 4.01104 57.5683 4.00933 49.0315 3.95021C45.4748 3.97219 42.155 3.96474 39.072 3.89988C34.0926 3.93066 29.3498 3.87606 24.8441 3.82C24.1328 3.82439 23.6584 3.79935 23.1842 3.80229C21.2874 3.84198 19.8644 3.79483 18.2042 3.74915C17.0187 3.75648 16.07 3.73437 15.3585 3.7108C14.647 3.68722 13.9357 3.69162 13.2241 3.66805C12.5125 3.6165 11.3269 3.62383 10.1413 3.63116C8.24405 3.58694 5.87274 3.57363 5.39765 3.4367C1.84077 3.43071 0.891628 3.32469 1.12788 3.18337C-0.295844 3.02433 -0.0597648 2.85504 1.12477 2.67988C1.12425 2.59597 1.12373 2.51205 1.12321 2.42814L1.12304 2.40017C0.647594 2.2073 3.2548 2.02334 2.54243 1.85991C2.30444 1.72152 3.72644 1.60084 4.6742 1.48309C4.67403 1.45512 4.67368 1.39918 5.62198 1.36534C5.85891 1.33591 6.5701 1.30354 7.51838 1.2697C8.70326 1.15049 10.1254 1.05778 12.4964 1.01515C12.9702 0.956276 13.9189 0.978385 14.6302 0.973987C15.3414 0.941619 16.0526 0.90925 16.7637 0.876881C17.4749 0.844513 18.1862 0.840116 18.8976 0.835718C21.7426 0.762187 24.5878 0.716626 27.6703 0.697572C28.8559 0.690243 29.8043 0.68438 30.9899 0.677052C31.4641 0.67412 32.1754 0.669723 32.6495 0.63882C39.0516 0.599246 45.6908 0.558205 52.0931 0.546602C55.4127 0.526082 58.7321 0.47759 62.0522 0.540985C64.8975 0.523396 68.2168 0.446933 71.2998 0.511793C72.0111 0.507395 72.7225 0.502998 73.1967 0.500067C75.3306 0.458904 77.2275 0.447178 79.3617 0.461958C84.5782 0.429712 89.7949 0.425438 94.7744 0.422629C96.4342 0.412369 98.331 0.372671 99.754 0.41982C100.703 0.441929 101.651 0.436066 102.599 0.402231C105.682 0.383177 108.764 0.364122 111.847 0.373039C112.321 0.370108 112.795 0.367177 113.27 0.364245C114.455 0.384888 115.878 0.348122 117.064 0.368765C118.723 0.358505 120.62 0.346779 122.28 0.336519C123.703 0.327725 125.363 0.317465 126.548 0.338107C127.497 0.332244 128.445 0.326382 129.394 0.320519C131.291 0.308793 133.425 0.26763 135.322 0.311847C138.879 0.289861 142.435 0.295847 145.755 0.275326C151.446 0.240149 157.137 0.260915 162.828 0.253709C164.725 0.241983 166.384 0.231723 168.282 0.247968C166.622 0.258229 164.725 0.269954 163.065 0.280214C164.725 0.241983 166.622 0.230257 168.282 0.247968C171.364 0.228914 174.209 0.211325 177.055 0.24968C178.241 0.242351 179.426 0.235022 180.612 0.255665C180.849 0.254199 181.086 0.252734 181.323 0.251268C183.932 0.235145 186.54 0.246993 189.149 0.286814C191.52 0.272156 193.654 0.314908 195.788 0.329688C196.974 0.322359 198.159 0.31503 199.345 0.363645C201.48 0.378425 203.377 0.39467 204.089 0.474188C205.511 0.40945 206.46 0.45953 207.645 0.480173C208.831 0.528788 208.358 0.615634 209.781 0.662782C209.069 0.667179 208.358 0.671577 207.647 0.675974C207.884 0.70248 208.121 0.728985 208.358 0.72752C210.018 0.773202 211.678 0.790914 213.338 0.808625C213.576 0.80716 213.813 0.805694 214.05 0.804228C215.71 0.793968 217.606 0.754271 219.029 0.801419L219.266 0.799954C219.266 0.799954 219.267 0.827925 219.029 0.829391C218.081 0.835254 217.133 0.841117 216.421 0.845514L216.184 0.84698C217.369 0.811679 218.318 0.805817 219.029 0.829391C219.504 0.938345 218.082 0.975111 217.608 1.03399C217.845 1.06049 216.66 1.09579 217.609 1.1179C218.32 1.1135 219.505 1.0782 219.268 1.13561C219.032 1.19302 217.609 1.17384 216.898 1.17824C216.66 1.17971 216.423 1.20914 216.186 1.21061C218.084 1.2828 219.981 1.29904 221.878 1.34326C224.723 1.32567 225.91 1.43023 227.57 1.50388C228.044 1.52892 228.756 1.49655 228.993 1.52306C227.57 1.53186 228.282 1.61137 227.334 1.64521C224.015 1.72167 224.015 1.80558 227.098 1.89842C228.047 1.92052 229.944 1.93677 230.419 2.01775C230.419 2.04572 230.419 2.0737 230.419 2.0737C230.893 2.07076 231.368 2.06783 231.368 2.0958C231.131 2.15321 230.419 2.12964 229.708 2.13404C229.234 2.13697 228.523 2.14137 228.286 2.1708C228.76 2.25179 228.523 2.28122 227.101 2.29002C227.575 2.34303 228.998 2.30626 228.761 2.39164C229.473 2.38725 229.947 2.41229 230.658 2.40789C229.947 2.46823 228.524 2.42108 228.287 2.47849C228.999 2.53003 228.999 2.55801 227.814 2.53736C226.865 2.54323 225.679 2.52258 226.865 2.59917C227.577 2.59477 228.525 2.58891 229.237 2.64045ZM6.81321 2.28107C6.57609 2.28254 6.57612 2.28254 6.57629 2.31051C6.57612 2.28254 6.81321 2.28107 6.81321 2.28107ZM66.5686 2.33128C66.8059 2.35779 66.8057 2.32982 67.0428 2.32835C66.8057 2.32982 66.5686 2.33128 66.5686 2.33128ZM96.2104 2.56764C95.9733 2.56911 95.9733 2.56911 96.2104 2.56764V2.56764ZM76.2909 2.38307C76.2908 2.3551 76.2908 2.3551 76.2909 2.38307V2.38307ZM65.1499 2.98342C64.9128 2.98489 64.9128 2.98489 64.6755 2.95838C64.6755 2.95838 64.4384 2.95985 64.4385 2.98782C64.6757 2.98635 64.9128 2.98489 65.1499 2.98342ZM38.828 2.78249C39.0649 2.75305 38.8278 2.75452 38.828 2.78249V2.78249ZM77.2442 3.16041C77.4814 3.15895 77.4814 3.15895 77.2442 3.16041V3.16041ZM72.5015 3.10581C72.2643 3.10728 72.2645 3.13525 71.7903 3.13818C72.0272 3.10874 72.2643 3.10728 72.5015 3.10581ZM27.4465 2.85284C27.4467 2.88081 27.4467 2.88081 27.4465 2.85284V2.85284ZM174.691 1.3832C174.691 1.3832 174.691 1.35523 174.454 1.38466C174.454 1.38466 174.454 1.38466 174.217 1.38613C174.454 1.38466 174.454 1.38466 174.691 1.3832ZM193.903 2.27145C193.903 2.24348 194.14 2.24201 194.14 2.21404C193.666 2.24494 193.666 2.24494 193.903 2.27145ZM138.421 3.00603C138.421 3.00603 138.184 3.00749 138.184 3.03547C138.421 3.034 138.895 3.03107 139.133 3.0296C138.895 3.0031 138.658 3.00456 138.421 3.00603ZM153.35 1.34728C153.349 1.31931 153.349 1.31931 153.35 1.34728C153.349 1.31931 153.112 1.32078 153.35 1.34728ZM189.393 1.51609C189.63 1.51463 189.63 1.51463 189.393 1.51609C189.63 1.48666 189.867 1.48519 189.867 1.45722C189.63 1.45868 189.393 1.46015 189.156 1.46162C189.393 1.48812 189.393 1.48812 189.393 1.51609ZM186.782 1.0567C186.545 1.08614 186.545 1.08614 186.782 1.0567V1.0567ZM118.269 3.52222C118.269 3.55019 118.269 3.55019 118.032 3.57962C118.269 3.57816 118.506 3.57669 118.743 3.57523C118.506 3.54872 118.506 3.54872 118.269 3.52222ZM166.624 0.677801C166.861 0.676335 166.861 0.676335 167.099 0.674869C167.099 0.674869 166.861 0.676335 166.624 0.677801ZM97.1478 0.771601C96.9107 0.773067 96.9107 0.773067 97.1478 0.771601V0.771601ZM224.734 3.03193C225.682 2.97013 226.394 2.9937 227.342 3.01581C226.631 3.02021 225.682 3.02607 224.734 3.03193ZM127.023 0.391119C127.26 0.389653 127.497 0.388187 127.735 0.414693C127.26 0.389653 127.023 0.391119 127.023 0.391119Z"
                                            fill="url(#paint0_linear_720_932)" />
                                        <defs>
                                            <linearGradient id="paint0_linear_720_932" x1="196.345" y1="-0.373061"
                                                x2="24.7083" y2="95.8315" gradientUnits="userSpaceOnUse">
                                                <stop offset="0" stop-color="#FF307C" />
                                                <stop offset="0.247843" stop-color="#A85CC5" />
                                                <stop offset="1" stop-color="#3075FF" />
                                            </linearGradient>
                                        </defs>
                                    </svg>
                                </span>
                            </span>
                        </h3>
                    </div>
                </div>
            </div>

            <div class="row">
                <div class="col-xl-4 col-lg-4 col-md-6 col-sm-6 mb-30 wow tpfadeUp" data-wow-duration=".9s"
                    data-wow-delay=".3s">
                    <div class="tp-choose-6__item-wrap">
                        <div class="tp-choose-6__item">
                            <div class="tp-choose-6__icon">
                                <span>
                                    <img src="{{ asset('data/monitoring.png') }}" alt="" style="width: 60px;">
                                </span>
                            </div>
                            <div class="tp-choose-6__content">
                                <h6 class="tp-choose-6__title">
                                    Our Vision
                                </h6>
                                <p style="text-align: justify; color: black;">
                                    To provide skillful Myanmar workers to all over the world according to the international
                                    labour standards and to be the most successful firm.
                                </p>
                                <br><br>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="col-xl-4 col-lg-4 col-md-6 col-sm-6 mb-30 wow tpfadeUp" data-wow-duration=".9s"
                    data-wow-delay=".5s">
                    <div class="tp-choose-6__item-wrap space">
                        <div class="tp-choose-6__item">
                            <div class="tp-choose-6__icon">
                                <span>
                                    <img src="{{ asset('data/target.png') }}" alt="" style="width: 60px;">
                                </span>
                            </div>
                            <div class="tp-choose-6__content">
                                <h6 class="tp-choose-6__title">
                                    Our mission
                                </h6>
                                <p style="text-align: justify; color: black;">
                                    To establish a seamless connection between skilled Myanmar workers and world-class
                                    companies situated in all over the world. Through this collaboration, we seek to
                                    cultivate mutual growth and prosperity for both parties involved.
                                </p>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="col-xl-4 col-lg-4 col-md-6 col-sm-6 mb-30 wow tpfadeUp" data-wow-duration=".9s"
                    data-wow-delay=".7s">
                    <div class="tp-choose-6__item-wrap">
                        <div class="tp-choose-6__item">
                            <div class="tp-choose-6__icon">
                                <span>
                                    <img src="{{ asset('data/market.png') }}" alt="" style="width: 60px;">
                                </span>
                            </div>
                            <div class="tp-choose-6__content">
                                <h6 class="tp-choose-6__title">
                                    Values
                                </h6>
                                <p style="text-align: justify; color: black;">
                                    Our values underscore our commitment to excellence
                                </p>

                                <ul>
                                    <li style="color: black;">
                                        <i class="fa-solid fa-circle-check"></i>
                                        Safety and Security
                                    </li>

                                    <li style="color: black;">
                                        <i class="fa-solid fa-circle-check"></i>
                                        Customer Priority
                                    </li>

                                    <li style="color: black;">
                                        <i class="fa-solid fa-circle-check"></i>
                                        Training Contribution
                                    </li>

                                    <li style="color: black;">
                                        <i class="fa-solid fa-circle-check"></i>
                                        Affordability
                                    </li>

                                    <li style="color: black;">
                                        <i class="fa-solid fa-circle-check"></i>
                                        Service Excellence
                                    </li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>



    <div class="tp-team-4__area pt-50 pb-30">
        <div class="container">

            <div class="row">
                <div class="col-xl-12">
                    <div class="tp-choose-6__section-box text-center mb-50">
                        <span class="tp-gradiant-section-subtitle">
                            Shwe Wutt Hmone Kha Co.,Ltd
                        </span>
                        <h3 class="tp-section-title-3">
                            Meet
                            <span class="p-relative">
                                Our Team Members
                                <span class="tp-title-shape">
                                    <svg width="233" height="5" viewBox="0 0 233 5" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M229.237 2.64045C229.948 2.63606 231.134 2.60076 231.845 2.6523C230.66 2.65963 229.474 2.69493 228.526 2.70079C228.052 2.75967 229.238 2.78031 229.712 2.80535C230.661 2.79949 231.609 2.79363 232.321 2.84517C231.61 2.87754 230.661 2.8834 229.713 2.88927C228.764 2.89513 227.579 2.87449 226.868 2.93483C227.579 2.9584 227.579 2.9584 227.579 3.01434C225.92 3.05258 225.921 3.2204 223.312 3.18058C222.364 3.18645 221.415 3.19231 220.467 3.19817C219.282 3.31739 217.86 3.43807 214.777 3.40118C213.829 3.43501 212.88 3.44088 211.932 3.41877C209.324 3.49083 206.715 3.47899 204.107 3.52308C203.87 3.55252 203.633 3.55398 203.396 3.52748C202.448 3.53334 201.499 3.5392 200.551 3.54507L200.314 3.54653L200.076 3.548C199.128 3.55386 198.417 3.58623 197.468 3.56412C194.149 3.58464 190.592 3.60663 187.272 3.65512C185.138 3.66831 183.478 3.6506 182.292 3.57401L182.055 3.57548L181.818 3.57694C181.107 3.63728 181.345 3.71973 178.974 3.73439C176.602 3.72108 174.231 3.73573 171.623 3.75186C169.963 3.73415 168.303 3.80035 166.644 3.78264C160.715 3.79131 154.788 3.82795 148.86 3.83662C143.643 3.86887 138.426 3.87314 133.21 3.87742C131.313 3.94509 129.416 3.90087 127.756 3.91113C118.271 3.94179 108.55 4.00188 99.0646 3.9766C94.7964 3.97501 90.5285 4.02936 86.2601 3.9998C82.4663 4.02326 78.9094 4.01727 75.1154 4.01275C66.3418 4.01104 57.5683 4.00933 49.0315 3.95021C45.4748 3.97219 42.155 3.96474 39.072 3.89988C34.0926 3.93066 29.3498 3.87606 24.8441 3.82C24.1328 3.82439 23.6584 3.79935 23.1842 3.80229C21.2874 3.84198 19.8644 3.79483 18.2042 3.74915C17.0187 3.75648 16.07 3.73437 15.3585 3.7108C14.647 3.68722 13.9357 3.69162 13.2241 3.66805C12.5125 3.6165 11.3269 3.62383 10.1413 3.63116C8.24405 3.58694 5.87274 3.57363 5.39765 3.4367C1.84077 3.43071 0.891628 3.32469 1.12788 3.18337C-0.295844 3.02433 -0.0597648 2.85504 1.12477 2.67988C1.12425 2.59597 1.12373 2.51205 1.12321 2.42814L1.12304 2.40017C0.647594 2.2073 3.2548 2.02334 2.54243 1.85991C2.30444 1.72152 3.72644 1.60084 4.6742 1.48309C4.67403 1.45512 4.67368 1.39918 5.62198 1.36534C5.85891 1.33591 6.5701 1.30354 7.51838 1.2697C8.70326 1.15049 10.1254 1.05778 12.4964 1.01515C12.9702 0.956276 13.9189 0.978385 14.6302 0.973987C15.3414 0.941619 16.0526 0.90925 16.7637 0.876881C17.4749 0.844513 18.1862 0.840116 18.8976 0.835718C21.7426 0.762187 24.5878 0.716626 27.6703 0.697572C28.8559 0.690243 29.8043 0.68438 30.9899 0.677052C31.4641 0.67412 32.1754 0.669723 32.6495 0.63882C39.0516 0.599246 45.6908 0.558205 52.0931 0.546602C55.4127 0.526082 58.7321 0.47759 62.0522 0.540985C64.8975 0.523396 68.2168 0.446933 71.2998 0.511793C72.0111 0.507395 72.7225 0.502998 73.1967 0.500067C75.3306 0.458904 77.2275 0.447178 79.3617 0.461958C84.5782 0.429712 89.7949 0.425438 94.7744 0.422629C96.4342 0.412369 98.331 0.372671 99.754 0.41982C100.703 0.441929 101.651 0.436066 102.599 0.402231C105.682 0.383177 108.764 0.364122 111.847 0.373039C112.321 0.370108 112.795 0.367177 113.27 0.364245C114.455 0.384888 115.878 0.348122 117.064 0.368765C118.723 0.358505 120.62 0.346779 122.28 0.336519C123.703 0.327725 125.363 0.317465 126.548 0.338107C127.497 0.332244 128.445 0.326382 129.394 0.320519C131.291 0.308793 133.425 0.26763 135.322 0.311847C138.879 0.289861 142.435 0.295847 145.755 0.275326C151.446 0.240149 157.137 0.260915 162.828 0.253709C164.725 0.241983 166.384 0.231723 168.282 0.247968C166.622 0.258229 164.725 0.269954 163.065 0.280214C164.725 0.241983 166.622 0.230257 168.282 0.247968C171.364 0.228914 174.209 0.211325 177.055 0.24968C178.241 0.242351 179.426 0.235022 180.612 0.255665C180.849 0.254199 181.086 0.252734 181.323 0.251268C183.932 0.235145 186.54 0.246993 189.149 0.286814C191.52 0.272156 193.654 0.314908 195.788 0.329688C196.974 0.322359 198.159 0.31503 199.345 0.363645C201.48 0.378425 203.377 0.39467 204.089 0.474188C205.511 0.40945 206.46 0.45953 207.645 0.480173C208.831 0.528788 208.358 0.615634 209.781 0.662782C209.069 0.667179 208.358 0.671577 207.647 0.675974C207.884 0.70248 208.121 0.728985 208.358 0.72752C210.018 0.773202 211.678 0.790914 213.338 0.808625C213.576 0.80716 213.813 0.805694 214.05 0.804228C215.71 0.793968 217.606 0.754271 219.029 0.801419L219.266 0.799954C219.266 0.799954 219.267 0.827925 219.029 0.829391C218.081 0.835254 217.133 0.841117 216.421 0.845514L216.184 0.84698C217.369 0.811679 218.318 0.805817 219.029 0.829391C219.504 0.938345 218.082 0.975111 217.608 1.03399C217.845 1.06049 216.66 1.09579 217.609 1.1179C218.32 1.1135 219.505 1.0782 219.268 1.13561C219.032 1.19302 217.609 1.17384 216.898 1.17824C216.66 1.17971 216.423 1.20914 216.186 1.21061C218.084 1.2828 219.981 1.29904 221.878 1.34326C224.723 1.32567 225.91 1.43023 227.57 1.50388C228.044 1.52892 228.756 1.49655 228.993 1.52306C227.57 1.53186 228.282 1.61137 227.334 1.64521C224.015 1.72167 224.015 1.80558 227.098 1.89842C228.047 1.92052 229.944 1.93677 230.419 2.01775C230.419 2.04572 230.419 2.0737 230.419 2.0737C230.893 2.07076 231.368 2.06783 231.368 2.0958C231.131 2.15321 230.419 2.12964 229.708 2.13404C229.234 2.13697 228.523 2.14137 228.286 2.1708C228.76 2.25179 228.523 2.28122 227.101 2.29002C227.575 2.34303 228.998 2.30626 228.761 2.39164C229.473 2.38725 229.947 2.41229 230.658 2.40789C229.947 2.46823 228.524 2.42108 228.287 2.47849C228.999 2.53003 228.999 2.55801 227.814 2.53736C226.865 2.54323 225.679 2.52258 226.865 2.59917C227.577 2.59477 228.525 2.58891 229.237 2.64045ZM6.81321 2.28107C6.57609 2.28254 6.57612 2.28254 6.57629 2.31051C6.57612 2.28254 6.81321 2.28107 6.81321 2.28107ZM66.5686 2.33128C66.8059 2.35779 66.8057 2.32982 67.0428 2.32835C66.8057 2.32982 66.5686 2.33128 66.5686 2.33128ZM96.2104 2.56764C95.9733 2.56911 95.9733 2.56911 96.2104 2.56764V2.56764ZM76.2909 2.38307C76.2908 2.3551 76.2908 2.3551 76.2909 2.38307V2.38307ZM65.1499 2.98342C64.9128 2.98489 64.9128 2.98489 64.6755 2.95838C64.6755 2.95838 64.4384 2.95985 64.4385 2.98782C64.6757 2.98635 64.9128 2.98489 65.1499 2.98342ZM38.828 2.78249C39.0649 2.75305 38.8278 2.75452 38.828 2.78249V2.78249ZM77.2442 3.16041C77.4814 3.15895 77.4814 3.15895 77.2442 3.16041V3.16041ZM72.5015 3.10581C72.2643 3.10728 72.2645 3.13525 71.7903 3.13818C72.0272 3.10874 72.2643 3.10728 72.5015 3.10581ZM27.4465 2.85284C27.4467 2.88081 27.4467 2.88081 27.4465 2.85284V2.85284ZM174.691 1.3832C174.691 1.3832 174.691 1.35523 174.454 1.38466C174.454 1.38466 174.454 1.38466 174.217 1.38613C174.454 1.38466 174.454 1.38466 174.691 1.3832ZM193.903 2.27145C193.903 2.24348 194.14 2.24201 194.14 2.21404C193.666 2.24494 193.666 2.24494 193.903 2.27145ZM138.421 3.00603C138.421 3.00603 138.184 3.00749 138.184 3.03547C138.421 3.034 138.895 3.03107 139.133 3.0296C138.895 3.0031 138.658 3.00456 138.421 3.00603ZM153.35 1.34728C153.349 1.31931 153.349 1.31931 153.35 1.34728C153.349 1.31931 153.112 1.32078 153.35 1.34728ZM189.393 1.51609C189.63 1.51463 189.63 1.51463 189.393 1.51609C189.63 1.48666 189.867 1.48519 189.867 1.45722C189.63 1.45868 189.393 1.46015 189.156 1.46162C189.393 1.48812 189.393 1.48812 189.393 1.51609ZM186.782 1.0567C186.545 1.08614 186.545 1.08614 186.782 1.0567V1.0567ZM118.269 3.52222C118.269 3.55019 118.269 3.55019 118.032 3.57962C118.269 3.57816 118.506 3.57669 118.743 3.57523C118.506 3.54872 118.506 3.54872 118.269 3.52222ZM166.624 0.677801C166.861 0.676335 166.861 0.676335 167.099 0.674869C167.099 0.674869 166.861 0.676335 166.624 0.677801ZM97.1478 0.771601C96.9107 0.773067 96.9107 0.773067 97.1478 0.771601V0.771601ZM224.734 3.03193C225.682 2.97013 226.394 2.9937 227.342 3.01581C226.631 3.02021 225.682 3.02607 224.734 3.03193ZM127.023 0.391119C127.26 0.389653 127.497 0.388187 127.735 0.414693C127.26 0.389653 127.023 0.391119 127.023 0.391119Z"
                                            fill="url(#paint0_linear_720_932)" />
                                        <defs>
                                            <linearGradient id="paint0_linear_720_932" x1="196.345" y1="-0.373061"
                                                x2="24.7083" y2="95.8315" gradientUnits="userSpaceOnUse">
                                                <stop offset="0" stop-color="#FF307C" />
                                                <stop offset="0.247843" stop-color="#A85CC5" />
                                                <stop offset="1" stop-color="#3075FF" />
                                            </linearGradient>
                                        </defs>
                                    </svg>
                                </span>
                            </span>
                        </h3>
                    </div>
                </div>
            </div>

            <div class="row">
                @foreach ($teams as $team)
                    <div class="col-xl-3 col-lg-3 col-md-6 col-sm-6 mb-30 wow tpfadeUp" data-wow-duration=".9s"
                        data-wow-delay=".3s">
                        <div class="tp-team-4__item">

                            <div class="tp-team-4__thumb-2 p-relative">
                                <img class="w-100" src="{{ $team->photo }}" alt=""
                                    style="width: 100%; height: auto; background-size: top; object-fit: cover;">
                                <div class="tp-team-4__social-wrapper">
                                    <div class="tp-team-4__social-box">
                                        <a href="#"><i class="fa-brands fa-facebook-f"></i></a>
                                        <a href="#"><i class="fa-brands fa-vimeo-v"></i></a>
                                        <a href="#"><i class="fa-brands fa-twitter"></i></a>
                                        <a href="#"><i class="fa-brands fa-linkedin-in"></i></a>
                                    </div>
                                </div>
                            </div>

                            <div class="tp-team-4__info-box text-center" style="padding-top: 20px;">
                                <div class="tp-team-4__info">
                                    <h6 class="tp-team-4__title">
                                        <a href="javascript::void(0)" style="color: black;;">
                                            {{ $team->name ?? '' }}
                                        </a>
                                    </h6>
                                    <span style="color: black;">
                                        {{ $team->position ?? '' }}
                                    </span>
                                </div>
                            </div>
                        </div>
                    </div>
                @endforeach
            </div>
        </div>
    </div>
@endsection
